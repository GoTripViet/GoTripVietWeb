...
class UserController {
  ...
  async updateMyProfile(req, res) {
    try {
      ...
      const updatedUser = await userService.updateUserProfile(userId, { fullName, phone });
  ...
  async updateMyPreferences(req, res) {
    try {
      ...
      const updatedUser = await userService.updateUserPreferences(userId, preferencesData);
      res.status(200).json(updatedUser);
    } catch (error) {
      res.status(400).json({ message: error.message });
    }
  ...
  async forgotPassword(req, res) {
    try {
      ...
      res.status(200).json({ message: 'If user exists, a reset link has been sent' });
    } catch (error) {
