...
export default function UnifiedSearch({ onSearch, locations = [] }) {
  ...
  const handleSearch = () => {
    ...
    if (onSearch) onSearch(searchData);
      ...
      if (startPoint && startPoint !== "Tất cả")
        params.append("from", startPoint);
      ...
      if (hotelRating)
        params.append("star_rating", hotelRating.replace(" sao", "")); // Chỉ lấy số (4)
  ...
  const startPointList = Array.from(
    new Set([...defaultStartPoints, ...locations.map((l) => l.title)]),
  );
  ...
  return (
    <div className="w-100 position-relative" style={{ zIndex: 100 }}>
      <div ref={containerRef} className="search-bar-container">
        {/* 1. ĐIỂM KHỞI HÀNH */}
        <div
          className="search-item start-point"
          ref={targetStart}
        ...
        >
          ...
          <div className="d-flex align-items-center gap-3">
            ...
            <div>
              ...
              <div
                className={`search-value ${!date && "text-muted fw-normal"}`}
              >
        ...
        {/* 4. TÙY CHỌN (Bộ lọc) */}
        <div
          className="search-item filter-opt"
          ref={targetFilter}
      ...
      >
        <Popover
          className="border-0 shadow-lg rounded-3 mt-2"
          style={{ width: "240px", maxHeight: "300px", overflowY: "auto" }}
        >
          <Popover.Body className="p-0 py-2">
            {startPointList.map((city) => (
              <div
                key={city}
                className="px-3 py-2 cursor-pointer hover-bg-light d-flex align-items-center gap-2"
                onClick={() => {
                  setStartPoint(city);
                  setShowStart(false);
                }}
      ...
      >
        <Popover
          className="border-0 shadow-lg rounded-4 mt-2"
          style={{ width: "350px" }}
        >
          ...
          <Popover.Body>
            ...
            <div className="mb-4">
              ...
              <div className="d-flex flex-wrap gap-2">
                {BUDGET_OPTIONS.map((opt) => (
                  <Button
                    key={opt}
                    variant={budget === opt ? "primary" : "outline-secondary"}
                    size="sm"
              ...
              <div className="d-flex flex-wrap gap-2">
                {HOTEL_RATINGS.map((opt) => (
                  <Button
                    key={opt}
                    variant={
                      hotelRating === opt ? "warning" : "outline-secondary"
                    }
                    size="sm"
                    className="rounded-pill"
                    onClick={() =>
                      setHotelRating(hotelRating === opt ? "" : opt)
                    }
                  >
                    {opt}
                  </Button>
              ...
              <div className="btn-group w-100">
                {TRANSPORTS.map((opt) => (
                  <Button
                    key={opt}
                    variant={
                      transport === opt ? "info text-white" : "outline-info"
                    }
                    size="sm"
            ...
            <div className="d-grid mt-3">
              <Button onClick={() => setShowFilter(false)} className="fw-bold">
                Áp dụng
              </Button>
            </div>
          </Popover.Body>
