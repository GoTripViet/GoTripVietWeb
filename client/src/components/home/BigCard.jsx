...
export default function BigCard({
  ...
  imageUrl,
  title,
  price,          // Vẫn nhận props để code không lỗi, nhưng không dùng để hiển thị
  originalPrice,  // Vẫn nhận props
  tourCode,       
  startPoint,    
  duration,      
  departureDates = [], 
  ...
  return (
    <div className="card h-100 border shadow-sm cursor-pointer hover-shadow" onClick={onClick}>
      ...
      <div className="card-body p-3 d-flex flex-column">
        {/* 2. TIÊU ĐỀ TOUR */}
        <h6 className="card-title fw-bold text-dark mb-3 text-truncate-2-lines" title={title}>
          {title}
        </h6>
        ...
        <div className="row g-2 mb-3 small text-secondary">
          <div className="col-6 d-flex align-items-center gap-2">
            ...
            <span className="text-truncate">Mã: <span className="fw-bold text-dark">{tourCode || "N/A"}</span></span>
          </div>
        ...
        <div className="d-flex align-items-center gap-2 mb-3">
          ...
          <span className="small text-muted me-1" style={{minWidth: '60px'}}>Khởi hành:</span>
          <div className="d-flex gap-1 overflow-hidden">
            {departureDates && departureDates.length > 0 ? (
              departureDates.slice(0, 3).map((date, index) => (
                <span key={index} className="border border-secondary text-secondary rounded px-2 py-0 small bg-light" style={{ fontSize: '11px', whiteSpace: 'nowrap' }}>
                  {formatDate(date)}
                </span>
              ))
            ) : (
            ...
            {departureDates.length > 3 && <span className="small text-muted">...</span>}
          </div>
        </div>
        ...
        <div className="mt-auto pt-3 border-top">
          ...
          <Button variant="outline-primary" size="sm" className="fw-bold w-100 rounded-pill hover-bg-primary">
            Xem chi tiết
          </Button>
        </div>
      </div>
