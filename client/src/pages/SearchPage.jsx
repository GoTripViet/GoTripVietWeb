  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (transport) apiParams.transport = transport;
        ...
        const mappedList = list.map(p => {
             ...
             const fakeCode = p._id ? `TOUR-${p._id.slice(-4).toUpperCase()}` : "TOUR-CODE";
             ...
             const displayDates = rawDates.slice(0, 3).map(d => formatShortDate(d));

             // Xử lý ảnh (dùng ảnh đầu tiên hoặc ảnh placeholder)
             const firstImage = (p.images && p.images.length > 0) ? p.images[0] : "";
             const validImage = (firstImage && firstImage.startsWith("http")) ? firstImage : "https://placehold.co/400x300?text=Tour+Image";
    ...
    fetchProducts();
  }, [keyword, startPoint, date, budget, transport, starRating]); 
    ...
    <Container className="py-5">
      ...
      {loading && (
        <div className="text-center py-5">
            ...
            <p className="mt-3 text-muted">Đang tìm kiếm tour tốt nhất cho bạn...</p>
        </div>
      )}
      ...
      {!loading && products.length === 0 && (
        <div className="text-center py-5 bg-light rounded-4 border border-dashed">
           ...
           <h4 className="fw-bold text-secondary">Không tìm thấy kết quả phù hợp</h4>
           <p className="text-muted mb-4">
               {date 
                 ? `Rất tiếc, chưa có tour nào khởi hành vào ngày ${formatDateFull(date)}.` 
                 : "Hãy thử thay đổi từ khóa, ngày đi hoặc mở rộng khoảng giá."}
           </p>
           <button onClick={() => navigate('/')} className="btn btn-primary px-4 py-2 fw-bold rounded-pill">
