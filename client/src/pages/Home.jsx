...
export default function Home() {
  ...
  const handleUnifiedSearch = (data) => {
    ...
    if (data.startPoint && data.startPoint !== 'Tất cả') params.append('from', data.startPoint);
    ...
    if (data.hotelRating) params.append('star_rating', data.hotelRating.replace(/\D/g, ''));
  ...
  useEffect(() => {
    const fetchHomeData = async () => {
      try {
        ...
        // b. Xử lý LOCATION
        let locList = Array.isArray(locationsRes?.data || locationsRes) ? (locationsRes?.data || locationsRes) : [];
        setRealLocations(locList.map(loc => ({
          id: loc._id, 
          title: loc.name, 
          subTitle: "Điểm đến hot",
          imageUrl: (loc.image?.startsWith('http')) ? loc.image : "https://placehold.co/200x200?text=Location"
        })));

        // c. Xử lý TOUR (Dùng mapProductToCard)
        const tourListRaw = Array.isArray(toursRes?.products || toursRes?.data?.products) 
        ...
        const rootCats = Array.isArray(rootCatsRes.data) ? rootCatsRes.data : (Array.isArray(rootCatsRes) ? rootCatsRes : []);
        
        // Load danh mục con cho từng danh mục cha
        const sectionsData = await Promise.all(rootCats.map(async (parentCat) => {
            try {
                const childrenRes = await catalogApi.getAllCategories({ parent: parentCat._id });
                ...
                const formattedChildren = childrenList.map(child => ({
                    ...
                    imageUrl: (child.image?.startsWith('http')) 
                        ? child.image 
                        : `https://placehold.co/300x300/e0f7fa/006064?text=${encodeURIComponent(child.name)}`
                }));
  ...
  return (
    <>
      ...
      <div className="py-5 bg-light shadow-sm mb-2" style={{backgroundImage: 'linear-gradient(to bottom, #f8f9fa, #e9ecef)'}}>
        <Container>
          <div className="text-center mb-4">
            <h2 className="fw-bold mb-2 text-primary">Khám phá thế giới cùng GoTripViet</h2>
            <p className="text-muted fs-5">Tìm tour du lịch trọn gói, giá tốt nhất dành cho bạn</p>
          </div>
          <UnifiedSearch 
      ...
      <Container className="my-4">
        <Event backgroundUrl={cld("event_boxingday_iusunh", { w: 1200, h: 450, crop: "fill", g: "auto" })} alt="Event" href="#" />
      </Container>
      <Container className="my-4">
        <Slider title="Ưu đãi & Sự kiện" items={events} itemMinWidth={350} renderItem={(e) => <Event {...e} />} />
      </Container>
      ...
      <Container className="my-5" ref={refLocations}>
        <Slider
          ...
          renderItem={(c) => (
            <SmallCard
              {...c}
              onClick={() => navigate(`/search?location=${c.id}&q=${encodeURIComponent(c.title)}`)}
            />
          )}
        />
      </Container>
         ...
         <Container className="my-5" key={section.parentId}>
            <Slider 
                ...
                renderItem={(childCat) => (
                    <SmallCard 
                        {...childCat} 
                        onClick={() => navigate(`/search?category=${childCat.id}&q=${encodeURIComponent(childCat.title)}`)} 
                    />
                )} 
            />
        </Container>
      ...
      {/* --- PHẦN 6: BANNER APP --- */}
      <div className="mt-5">
         <BannerMobile backgroundUrl="/assets/app/app_bg.jpg" title="Tải ứng dụng ngay" />
      </div>
    </>
  );
}