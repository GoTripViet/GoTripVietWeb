...
export default function ProductDetail() {
    ...
    const handleBookingClick = () => {
        ...
        const transportType = product.tour_details?.transport_type || "Đang cập nhật";
            ...
            transportInfoData = {
                ...
                details: {
                    ...
                    depart: {
                        ...
                        location: schedule.pickup_location || product.tour_details?.start_point || "Đang cập nhật"
                    }
                }
            };
        }
        ...
        navigate("/order", {
            state: {
                product: {
                    ...
                    code: product.product_code || `TOUR-${product._id.slice(-6).toUpperCase()}`,

                    // [QUAN TRỌNG] Giá lấy từ Inventory (Giá chính xác theo ngày)
                    basePrice: selectedInventory ? selectedInventory.price : product.base_price,
    ...
    if (!product) return <div className="text-center py-5"><h3>Không tìm thấy tour!</h3></div>;

    const t = product.tour_details || {};
    const sortedDates = t.departure_times ? [...t.departure_times].sort((a, b) => new Date(a) - new Date(b)) : [];
        ...
        <Container className="py-5">
            ...
            <div className="mb-4">
                ...
                <div className="d-flex flex-wrap align-items-center gap-3 text-muted mb-3 small">
                    ...
                    <span>|</span>
                    <span className="text-primary fw-bold bg-light px-2 py-1 rounded">MÃ: {product._id.slice(-6).toUpperCase()}</span>
                </div>
                <div className="rounded-4 overflow-hidden position-relative shadow-sm" style={{ height: '450px' }}>
                    <img src={product.images?.[0] || "https://placehold.co/1200x600"} alt={product.title} className="w-100 h-100 object-fit-cover hover-zoom" />
                </div>
            </div>
                                ...
                                <table className="table table-hover mb-0 align-middle">
                                    ...
                                    <tbody>
                                        {inventoryItems.map((item) => {
                                            ...
                                            const availableSlots = item.tour_details.total_slots - item.tour_details.booked_slots;
                                            const isSelected = selectedInventory && selectedInventory._id === item._id;
                                            return (
                                                <tr key={item._id} className={isSelected ? "table-primary" : ""}>
                                                    <td className="ps-4"><div className="fw-bold text-primary">{formatDateWithWeekday(dateStr)}</div></td>
                                                    {/* GIÁ TỪ INVENTORY */}
                                                    <td className="fw-bold text-danger fs-5">{formatCurrency(item.price)}</td>
                                                    <td>{availableSlots > 0 ? <Badge bg="success" className="rounded-pill">Còn {availableSlots} chỗ</Badge> : <Badge bg="secondary">Hết chỗ</Badge>}</td>
                                                    <td className="text-end pe-4">
                                                        <Button variant={isSelected ? "primary" : "outline-primary"} size="sm" className="rounded-pill px-3 fw-bold" disabled={availableSlots <= 0}
                                                            onClick={() => { setSelectedDate(dateStr); setSelectedInventory(item); }}>
                                                            {isSelected ? <><i className="bi bi-check2"></i> Đã chọn</> : "Chọn"}
                                                        </Button>
                                                    </td>
                                                </tr>
                                            );
                        ...
                        <Accordion defaultActiveKey="0" className="shadow-sm rounded overflow-hidden custom-accordion">
                            {t.itinerary?.map((item, idx) => (
                                <Accordion.Item eventKey={idx.toString()} key={idx} className="border-bottom">
                                    <Accordion.Header><div className="fw-bold text-dark"><span className="badge bg-primary rounded-pill me-2">Ngày {item.day}</span>{item.title}</div></Accordion.Header>
                                    <Accordion.Body className="bg-light bg-opacity-10 pt-3 pb-4">
                                        <div className="mb-4 text-secondary ps-2 border-start border-2 border-light" style={{ whiteSpace: 'pre-line', lineHeight: '1.7', textAlign: 'justify' }}>
                                        ...
                                        <div className="d-inline-flex align-items-center gap-2 px-3 py-2 rounded-3 bg-white border shadow-sm">
                                            <i className="bi bi-cup-hot-fill text-warning fs-5"></i><span className="fw-bold text-dark small">Bữa ăn:</span><span className="text-muted small fw-medium">{renderMeals(item.meals)}</span>
                                        </div>
                                    </Accordion.Body>
                                </Accordion.Item>
                            ))}
                        ...
                        <Card className="border-0 shadow-lg rounded-4 overflow-hidden">
                            ...
                            <Card.Body className="p-4">
                                ...
                                <div className="mb-4">
                                    ...
                                    <Form.Select
                                        ...
                                        onChange={(e) => {
                                            ...
                                            const invItem = inventoryItems.find(i => i.tour_details.date === dateVal);
                                            if (invItem) {
                                    ...
                                    >
                                        {inventoryItems.length === 0 && <option value="" disabled>-- Hết chỗ --</option>}
                                        {inventoryItems.map((item, i) => {
                                            const available = item.tour_details.total_slots - item.tour_details.booked_slots;
                                            return (
                                                <option key={i} value={item.tour_details.date} disabled={available <= 0}>
                                                    {formatDateWithWeekday(item.tour_details.date)} {available <= 0 ? '(Hết)' : ''}
                                                </option>
                                            );
                                        })}
                                    </Form.Select>
                                ...
                                <div className="mb-4 text-center">
                                    <div className="small text-muted mb-1">Giá tour trọn gói / khách</div>
                                    <div className="fs-1 fw-bold text-danger lh-1">
                                        ...
                                        {formatCurrency(selectedInventory ? selectedInventory.price : product.base_price)}
                                    </div>
                                    <div className="small text-success mt-1"><i className="bi bi-check-circle-fill"></i> Đã bao gồm thuế & phí</div>
                                </div>

                                <Button
                                    variant="danger" size="lg" className="w-100 fw-bold py-3 text-uppercase shadow hover-scale"
                    ...
                    <Row>
                        {relatedTours.map((item) => (
                            <Col key={item.id} xs={12} md={6} lg={3} className="mb-4">
                                <BigCard {...item} onClick={() => navigate(`/product/${item.id}`)} />
                            </Col>
                        ))}
                    </Row>
                </div>
