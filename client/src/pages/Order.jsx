...
export default function Order() {
  ...
  useEffect(() => {
    const loadPromos = async () => {
      try {
        const res = await inventoryApi.getActivePromotionsPublic();
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < counts.toddler; i++)
      newPassengers.push({
        type: "toddler",
        label: "Trẻ nhỏ",
        index: i,
      ...
      if (promo.rules && promo.rules.min_spend > subTotal) {
        setPromoError(
          `Đơn hàng phải từ ${formatCurrency(
            promo.rules.min_spend
          )} mới được dùng mã này.`
        );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const departDate =
        transportInfo?.details?.depart?.date ||
        ...
        items: [
          {
            ...
            productType: "tour",
            quantity:
              counts.adult + counts.child + counts.toddler + counts.infant,
    ...
    if (!transportInfo || transportInfo.type === "other") {
      return (
        <div className="p-3 border-bottom bg-light bg-opacity-50 text-muted small fst-italic">
          ...
          Phương tiện:{" "}
          {transportInfo?.details?.vehicle || "Theo lịch trình tour"}
        </div>
      );
    }
    ...
    return (
      <div className="p-3 border-bottom bg-light bg-opacity-50">
        <div className="fw-bold small mb-2 text-primary">
          {isFlight ? (
            <>
              <i className="bi bi-airplane me-1"></i> THÔNG TIN CHUYẾN BAY
            </>
          ) : (
            <>
        ...
        <div className="mb-3">
          ...
          <div className="d-flex justify-content-between fw-bold small">
            ...
            <span>
              {isFlight ? depart.code : depart.location || "Điểm hẹn"}
            </span>
          </div>
        </div>
          ...
          <div className="bg-light p-3 rounded mb-4 border">
            {!isLoggedIn && (
              <div className="bg-white p-2 mb-3 rounded border border-info text-primary d-flex align-items-center gap-2">
                ...
                <span>
                  <strong>Đăng nhập</strong> để nhận ưu đãi và quản lý đơn hàng!
                </span>
              </div>
            )}
                ...
                <div className="qty-box bg-white h-100 p-3 border rounded d-flex justify-content-between align-items-center shadow-sm">
                  ...
                  <div className="d-flex align-items-center gap-3">
                    <Button
                      ...
                      disabled={counts[item.key] <= item.min}
                    >
                      -
                    </Button>
          ...
          <div className="bg-white border rounded p-3 mb-4">
            {passengers.map((p, idx) => (
              <div
                key={idx}
                className={`mb-3 ${
                  idx < passengers.length - 1 ? "border-bottom pb-3" : ""
                }`}
              >
          ...
          <div className="summary-card bg-white overflow-hidden shadow rounded border">
            <div className="p-3 border-bottom">
              ...
              <div className="d-flex gap-2">
                ...
                />
                <div>
                  <div className="fw-bold small text-truncate-2-lines">
                    {productData.title}
                  </div>
            ...
            <div className="p-3">
              ...
              <div className="small mb-1 d-flex justify-content-between">
                <span>Người lớn</span>
                <span>
                  {counts.adult} x {formatCurrency(productData.basePrice)}
                </span>
              </div>
              ...
              {counts.child > 0 && (
                <div className="small mb-1 d-flex justify-content-between">
                  ...
                  <span>
                    {counts.child} x{" "}
                    {formatCurrency(productData.basePrice * 0.8)}
                  </span>
                </div>
              )}
              ...
              {counts.toddler > 0 && (
                <div className="small mb-1 d-flex justify-content-between">
                  ...
                  <span>
                    {counts.toddler} x{" "}
                    {formatCurrency(productData.basePrice * 0.5)}
                  </span>
                </div>
              )}
              ...
              {counts.infant > 0 && (
                <div className="small mb-1 d-flex justify-content-between">
                  ...
                  <span>
                    {counts.infant} x{" "}
                    {formatCurrency(productData.basePrice * 0.1)}
                  </span>
                </div>
              )}
              ...
              <div className="mt-3 bg-light p-3 rounded border border-dashed">
                  ...
                  <div className="d-flex flex-wrap gap-2 mt-2">
                    {promotions.slice(0, 6).map((p) => (
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          setPromoCode((p.code || "").toUpperCase());
                          // auto apply luôn:
                          setTimeout(() => handleApplyPromo(), 0);
                        }}
                      >
                        {p.code}{" "}
                        {p.type === "percentage"
                ...
                {appliedPromo && (
                  <div className="d-flex justify-content-between text-success small fw-bold">
                    <span>
                      <i className="bi bi-check-circle"></i> {appliedPromo.code}
                    </span>
              ...
              <Button
                variant="danger"
                size="lg"
                className="w-100 fw-bold shadow hover-scale"
