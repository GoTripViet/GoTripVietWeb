// src/components/SearchingBar.tsx
import React, { useState } from "react";

export type SearchingBarProps = {
  className?: string;
};
...
const propertyTypes: CheckboxFilter[] = [
...
const roomAmenities: CheckboxFilter[] = [
...
const reviewScores: CheckboxFilter[] = [
...
const starRatings: CheckboxFilter[] = [
...
const SearchingBar: React.FC<SearchingBarProps> = ({ className }) => {
  ...
  const handleBudgetMin = (v: number) => {
    setBudgetMin(Math.min(v, budgetMax));
  };

  const handleBudgetMax = (v: number) => {
    setBudgetMax(Math.max(v, budgetMin));
  };

  const renderCheckboxGroup = (title: string, items: CheckboxFilter[]) => (
    <div className="mb-3">
...
//     {/* danh sách khách sạn ở bên phải */}
//   </div>
// </div>