// src/components/SmallCard.tsx
import React from "react";

export type SmallCardProps = {
  imageUrl: string;
  /** Loại lưu trú: "Nhà nghỉ", "Nhà nghỉ giữa thiên nhiên", "Khách sạn", ... */
  stayType: string;
  /** Số sao (nếu có) */
  stars?: number;
  /** Tên khách sạn/homestay */
  name: string;

  /** Điểm đánh giá 0–10 */
  ratingScore?: number;
  /** Số lượt đánh giá */
  reviewCount?: number;
  /** Số km tới trung tâm */
  distanceToCenterKm?: number;

  /** Giá bắt đầu từ (mặc định VND) */
  priceFrom: number;
  currency?: string;

  /** Trái tim yêu thích */
  isFavorite?: boolean;
  onToggleFavorite?: () => void;

  /** Click cả card */
  onClick?: () => void;
};

function formatCurrency(value: number, currency = "VND") {
  return (
    currency +
    " " +
    value.toLocaleString("vi-VN", {
      maximumFractionDigits: 0,
    })
  );
}

function getRatingLabel(score?: number) {
  if (score == null) return "";
  if (score >= 9) return "Tuyệt hảo";
  if (score >= 8) return "Rất tốt";
  if (score >= 7) return "Tốt";
  return "Đánh giá";
}

const SmallCard: React.FC<SmallCardProps> = ({
  imageUrl,
  stayType,
  stars,
  name,
  ratingScore,
  reviewCount,
  distanceToCenterKm,
  priceFrom,
  currency = "VND",
  isFavorite,
  onToggleFavorite,
  onClick,
}) => {
  const score =
    typeof ratingScore === "number"
      ? Math.round(ratingScore * 10) / 10
      : undefined;
  const ratingLabel = getRatingLabel(ratingScore);

  return (
    <article
      className="card border-0 shadow-sm h-100"
      style={{ borderRadius: "1rem", overflow: "hidden", cursor: onClick ? "pointer" : "default" }}
      onClick={onClick}
    >
      {/* Ảnh + heart */}
      <div className="position-relative">
        <img
          src={imageUrl}
          alt={name}
          className="w-100"
          style={{ height: 180, objectFit: "cover" }}
        />
        <button
          type="button"
          className="btn btn-light rounded-circle shadow position-absolute top-0 end-0 m-2 p-2"
          onClick={(e) => {
            e.stopPropagation();
            onToggleFavorite?.();
          }}
        >
          <i
            className={
              isFavorite ? "bi bi-heart-fill text-danger" : "bi bi-heart"
            }
          />
        </button>
      </div>

      <div className="card-body pb-3">
        {/* Loại lưu trú + sao */}
        <div className="small text-muted mb-1">
          {stayType}
          {typeof stars === "number" && stars > 0 && (
            <>
              <br />
              <span className="text-warning">
                {"★".repeat(stars)}
              </span>
            </>
          )}
        </div>

        {/* Tên hotel */}
        <h6 className="fw-bold mb-2 text-dark">{name}</h6>

        {/* Rating */}
        {(score != null || ratingLabel || reviewCount) && (
          <div className="d-flex align-items-center mb-2">
            {score != null && (
              <span
                className="badge rounded-3 me-2"
                style={{
                  backgroundColor: "#003b95",
                  color: "#fff",
                  minWidth: 32,
                  textAlign: "center",
                }}
              >
                {score.toFixed(1)}
              </span>
            )}
            <div className="small">
              {ratingLabel && <span className="fw-semibold">{ratingLabel}</span>}
              {typeof reviewCount === "number" && (
                <span className="text-muted">
                  {" · "}
                  {reviewCount.toLocaleString("vi-VN")} đánh giá
                </span>
              )}
            </div>
          </div>
        )}

        {/* Khoảng cách tới trung tâm */}
        {typeof distanceToCenterKm === "number" && (
          <div className="small text-muted mb-3 d-flex align-items-center">
            <i className="bi bi-geo-alt me-1" />
            <span>{distanceToCenterKm.toLocaleString("vi-VN")}km từ trung tâm</span>
          </div>
        )}

        {/* Giá bắt đầu từ */}
        <div className="mt-auto small">
          <span className="text-muted">Bắt đầu từ </span>
          <span className="fw-bold text-dark">
            {formatCurrency(priceFrom, currency)}
          </span>
        </div>
      </div>
    </article>
  );
};

export default SmallCard;

{/* <SmallCard
  imageUrl="/images/green-hope.jpg"
  stayType="Nhà nghỉ giữa thiên nhiên"
  stars={2}
  name="Green Hope Lodge"
  ratingScore={9.0}
  reviewCount={728}
  distanceToCenterKm={3.1}
  priceFrom={770000}
  isFavorite={false}
  onToggleFavorite={() => console.log("toggle")}
  onClick={() => console.log("open detail")}
/> */}
