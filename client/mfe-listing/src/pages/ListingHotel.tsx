// src/pages/ListingHotel.tsx
import React from "react";
import { cld } from "../utils/cld.ts";
import SearchingBar from "../components/SearchingBar.tsx";
import BestChoiceSearch from "../components/BestChoiceSearch.tsx";
import Slider from "../components/Slider.tsx";
import type { SmallCardProps } from "../components/SmallCard.tsx";
import ListingCard, {
  ListingCardProps,
} from "../components/HotelCard.tsx"; // hoặc "../components/Card" tuỳ bạn đặt tên file

// ====== DUMMY DATA – bạn có thể thay bằng data thật từ API ======
const SIMILAR_STAYS: SmallCardProps[] = [
  {
...
const LISTING_HOTELS: ListingCardProps[] = [
  {
    imageUrl: cld("hotel_danang_pwadaf", { w: 640, h: 480 }),
    ...
    payAtProperty: true,
    remainingRoomsText:
      "Chỉ còn 1 phòng với giá này trên trang của chúng tôi",
    priceInfo: {
      basePrice: 900000,
  ...
  },
  {
    ...
    payAtProperty: true,
    remainingRoomsText:
      "Chỉ còn 3 phòng với giá này trên trang của chúng tôi",
    priceInfo: {
      basePrice: 770000,
      currency: "VND",
      nights: 1,
...
export type ListingHotelProps = {
  destinationName?: string;
  totalStays?: number;
  hotels?: ListingCardProps[];
  similarStays?: SmallCardProps[];
};

const ListingHotel: React.FC<ListingHotelProps> = ({
        ...
        <div className="col-12 col-lg-9">
          ...
          {mainHotel && (
            <ListingCard
              {...mainHotel}
              onViewAvailability={() =>
                console.log("View availability:", mainHotel.title)
              }
            />
          )}
          ...
          {similarStays.length > 0 && (
            <div className="mt-3 mb-3">
              <Slider
                ...
                description={`Những chỗ nghỉ tương tự với ${mainHotel?.title ?? ""}`}
                items={similarStays}
              />
            </div>
          )}
          ...
          <div className="mt-3">
            {otherHotels.map((hotel) => (
              <ListingCard
                ...
                {...hotel}
                onViewAvailability={() =>
                  console.log("View availability:", hotel.title)
                }
              />
            ))}
          </div>
