// mfe-home/src/components/HotelSearch.tsx
...
type HotelSearchProps = {
  ...
  popular?: { name: string; subtitle?: string }[];
  className?: string;
};

const DEFAULT_POPULAR: NonNullable<HotelSearchProps["popular"]> = [
...
function buildMonth(year: number, month: number) {
  ...
  const startIdx = (first.getDay() + 6) % 7; // Mon=0 .. Sun=6
...
function CalendarRange({
  ...
  const [base, setBase] = useState(new Date(today.getFullYear(), today.getMonth(), 1)); // left month
  ...
  return (
    <div style={{ width: "min(560px, 95vw)" }}>
      <div className="d-flex justify-content-between align-items-center px-2 mb-2">
        <Button variant="light" size="sm" onClick={() => setBase(new Date(base.getFullYear(), base.getMonth() - 1, 1))}>
          ‹
        </Button>
      ...
      <div className="d-flex justify-content-between align-items-center border-top pt-2 px-2">
        <div className="small text-muted">
          Nhận phòng: <strong>{fmtDate(value.start) || "--"}</strong> &nbsp;–&nbsp; Trả phòng:{" "}
          <strong>{fmtDate(value.end) || "--"}</strong>
        </div>
        <div className="d-flex gap-2">
          <Button variant="light" size="sm" onClick={() => onChange({ start: undefined, end: undefined })}>
            Xoá
          </Button>
...
export default function HotelSearch({ onSearch, popular = DEFAULT_POPULAR, className }: HotelSearchProps) {
    // state
  const wrapRef = useRef<HTMLDivElement | null>(null);
  const popularList = popular ?? DEFAULT_POPULAR;
  const [destination, setDestination] = useState("");
  const [recent, setRecent] = useState<{ name: string; note?: string }[]>([]);
  ...
  function submit() {
    ...
    if (destination) {
      setRecent([{ name: destination, note: guestsLabel }, ...recent.slice(0, 4)]);
    }
  }
    ...
    <div ref={wrapRef} className={`gv-search p-2 rounded-3 ${className || ""}`}>
      <div className="row g-2 align-items-stretch">
        ...
        <div className="col-12 col-lg">
          <div
            ...
            role="button"
            onClick={() => { setShowDest(true); setShowDates(false); setShowGuests(false); }}
          >
            <i className="bi bi-building fs-5 me-2" />
            <div className="text-truncate">
              <div className="text-muted small">Tên thành phố, khách sạn hoặc địa điểm</div>
              <div className={destination ? "" : "text-muted"}>{destination || "Bạn muốn đến đâu?"}</div>
            </div>
          </div>
        </div>
        ...
        <div className="col-12 col-md-6 col-lg">
          <div
            ...
            className="h-100 bg-white rounded-3 border d-flex align-items-center px-3 py-2"
            role="button"
            onClick={() => { setShowDates(true); setShowDest(false); setShowGuests(false); }}
          >
            <i className="bi bi-calendar3 fs-5 me-2" />
            <div className="text-truncate">
              <div className="text-muted small">Ngày nhận phòng và trả phòng</div>
              <div className={range.start && range.end ? "" : "text-muted"}>{dateLabel}</div>
            </div>
          </div>
        </div>
        ...
        <div className="col-12 col-md-6 col-lg">
          <div
            ...
            className="h-100 bg-white rounded-3 border d-flex align-items-center px-3 py-2"
            role="button"
            onClick={() => { setShowGuests(true); setShowDest(false); setShowDates(false); }}
          >
            ...
            <div className="text-truncate">
              ...
              <div className="d-flex align-items-center gap-1 text-truncate">
                ...
                {pets && <span aria-label="Có thú cưng" title="Có thú cưng">🐱</span>}
              </div>
            </div>
      ...
      <Overlay
        ...
        popperConfig={{
          strategy: "absolute",
          modifiers: [
            { name: "offset", options: { offset: [0, 8] } },               // cách ô 8px
      ...
      >
        {(props) => (
          <Popover {...props} className={`shadow ${(props as any).className ?? ""}`} style={{ ...(props as any).style, minWidth: "min(420px, 92vw)", zIndex: 1080, maxWidth: "none" }}>
            ...
            <Popover.Body>
              ...
              {!!recent.length && (
                <>
                  <div className="fw-semibold mb-2">Tìm kiếm gần đây của bạn</div>
                    ...
                    {recent.map((r, i) => (
                      ...
                      >
                        ...
                        <div>
                          ...
                          {r.note && <div className="text-muted small">{r.note}</div>}
                        </div>
                      </button>
                    ))}
                  </div>
      ...
      <Overlay
        ...
        {(props) => (
          <Popover {...props} className={`shadow ${(props as any).className ?? ""}`} id="date-pop" style={{ ...(props as any).style, maxWidth: "none", width: "min(600px, 95vw)" }}>
            <Popover.Header className="d-flex align-items-center justify-content-between">
              <div className="fw-semibold">Ngày ở</div>
              <CloseButton onClick={() => setShowDates(false)} />
            </Popover.Header>
            <Popover.Body>
              <CalendarRange value={range} onChange={setRange} onDone={() => setShowDates(false)} />
            </Popover.Body>
          </Popover>
        )}
      </Overlay>
      ...
      >
        {(props) => (
          <Popover {...props} className={`shadow ${(props as any).className ?? ""}`} id="guests-pop" style={{ ...(props as any).style, minWidth: "min(360px, 92vw)", maxWidth: "none" }}>
            <Popover.Header className="d-flex align-items-center justify-content-between">
              ...
              {[
                ...
                <div key={row.label} className="d-flex align-items-center justify-content-between mb-2">
                  ...
                  <div className="d-flex align-items-center gap-2">
                    ...
                    <Button variant="light" onClick={() => row.set(row.val + 1)}>
                      +
                    </Button>
                  </div>
                </div>
