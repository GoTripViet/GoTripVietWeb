...
type TaxiAirportSearchProps = {
  onSearch?: (r: TaxiSearchResult) => void;
  className?: string;
};
...
export default function TaxiAirportSearch({
  onSearch,
  className,
}: TaxiAirportSearchProps) {
  ...
  return (
    ...
    >
      ...
      {/* Hàng chính – 1 thanh liền nhau, viền vàng */}
      <div className="border border-warning rounded-3 bg-white overflow-hidden">
        <div className="d-grid align-items-stretch taxi-search-grid">
            ...
            <div
                ...
                onClick={() => { setShowPickup(true); setShowDest(false); }}
            >
                <div className="text-truncate">
                <div className="text-muted small">Enter pick-up location</div>
                <div className={pickup ? "" : "text-muted"}>{pickup || "Enter pick-up location"}</div>
                </div>
            </div>
            </div>
            ...
            <div className="h-100 d-flex">
                <div className="d-flex align-items-center px-2 border-end"><i className="bi bi-arrow-left-right" /></div>
                <div
                ref={destRef}
                role="button"
                className="flex-grow-1 d-flex align-items-center px-2 py-2"
                onClick={() => { setShowDest(true); setShowPickup(false); }}
                >
                <div className="text-truncate">
                    <div className="text-muted small">Enter destination</div>
                    <div className={destination ? "" : "text-muted"}>{destination || "Enter destination"}</div>
                </div>
                </div>
            </div>
            </div>
            ...
            <div
                ...
                role="button"
                className="h-100 d-flex align-items-center px-2 py-2"
                onClick={() => { setShowDepart(true); setShowReturn(false); }}
            >
                <i className="bi bi-calendar3 fs-5 me-2" />
                <div className="text-truncate">
                <div className="text-muted small">Depart</div>
                <div className={departDate ? "" : "text-muted"}>{departLabel}</div>
                </div>
            </div>
            </div>
            ...
            <div
                ...
                role="button"
                className="h-100 d-flex align-items-center px-2 py-2"
                onClick={() => { setOneWay(false); setShowReturn(true); setShowDepart(false); }}
            >
                <i className="bi bi-calendar3 fs-5 me-2" />
                <div className="text-truncate">
                <div className="text-muted small">{oneWay && !returnDate ? "Add a return" : "Return"}</div>
                <div className={returnDate ? "" : "text-muted"}>{returnLabel}</div>
                </div>
            </div>
            </div>
