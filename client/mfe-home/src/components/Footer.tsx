  ...
  columns = [
    {
      title: "Sản phẩm",
      links: [
        { label: "Lưu trú" }, { label: "Chuyến bay" }, { label: "Chuyến bay & Khách sạn" },
        { label: "Đưa đón sân bay" }, { label: "Thuê xe" }, { label: "Hoạt động" },
      ],
    },
    {
      title: "Về chúng tôi",
      links: [{ label: "Về GoTripViet" }, { label: "Cách đặt chỗ" }, { label: "Liên hệ chúng tôi" }, { label: "Trợ giúp" }],
    },
    {
      title: "Điều khoản và cài đặt",
      links: [{ label: "Bảo mật và Cookie" }, { label: "Điều khoản dịch vụ" }, { label: "Tranh chấp đối tác" }],
    },
  ],
          ...
          {columns.map((col, i) => (
            <Col key={i} md={3}>
              ...
              <ul className="list-unstyled d-grid gap-2">
                {col.links.map((l, j) => (
                  <li key={j}><a className="text-white text-decoration-none opacity-85" href={l.href || "#"}>{l.label}</a></li>
                ))}
              </ul>
            </Col>
          ))}
