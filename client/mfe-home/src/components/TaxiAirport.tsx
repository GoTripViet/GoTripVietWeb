...
type TaxiClass = {
  id: string;
  label: string;          // Hạng chuẩn, Hạng sang...
  carExample: string;     // Skoda Octavia hoặc tương tự
  passengers: number;     // số hành khách tối đa
  luggage: number;        // số hành lý tiêu chuẩn
  includesMeetAndGreet?: boolean;
  freeCancel?: boolean;
};

const TAXI_CLASSES: TaxiClass[] = [
  {
    id: "standard-sedan",
    label: "Hạng chuẩn",
...
type TaxiAirportProps = {
  className?: string;
};

const TaxiAirport: React.FC<TaxiAirportProps> = ({ className }) => {
  const [activeTab, setActiveTab] = React.useState<"1-3" | "4-7" | "all">("1-3");

  const tabBtnClass = (value: "1-3" | "4-7" | "all") =>
