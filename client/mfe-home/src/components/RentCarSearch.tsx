// src/components/RentCarSearch.tsx
...
function buildMonth(year: number, month: number) {
  ...
  for (let i = 0; i < startIdx; i++)
    cells.push({ date: new Date(year, month, 1 - (startIdx - i)), inMonth: false });
  for (let d = 1; d <= daysInMonth; d++)
    cells.push({ date: new Date(year, month, d), inMonth: true });
  while (cells.length % 7 !== 0)
    cells.push({ date: new Date(year, month + 1, cells.length % 7), inMonth: false });
...
/** format "10:30" -> "10:30 SA/CH" */
function fmtTimeLabel(value: string) {
...
/** ===== Kết quả submit ===== */
export type RentCarSearchResult = {
  pickup: string;
  dropoff: string;
  differentLocation: boolean;
  datePick?: Date;
  timePick: string; // HH:mm
  dateDrop?: Date;
  timeDrop: string; // HH:mm
  driverAge?: number; // undefined nếu tick 30–65
};

type Props = {
  onSearch?: (r: RentCarSearchResult) => void;
  className?: string;
};

export default function RentCarSearch({ onSearch, className }: Props) {
  ...
  const [datePick, setDatePick] = useState<Date | undefined>(undefined);
  const [dateDrop, setDateDrop] = useState<Date | undefined>(undefined);

  const [timePick, setTimePick] = useState("10:00");
  const [timeDrop, setTimeDrop] = useState("10:00");

  const [in3065, setIn3065] = useState(false);
  const [driverAge, setDriverAge] = useState<number | undefined>(undefined);

  // overlay anchors
  const datePickRef = useRef<HTMLDivElement | null>(null);
  const dateDropRef = useRef<HTMLDivElement | null>(null);
  const timePickRef = useRef<HTMLDivElement | null>(null);
  const timeDropRef = useRef<HTMLDivElement | null>(null);

  const [showPickDate, setShowPickDate] = useState(false);
  const [showDropDate, setShowDropDate] = useState(false);

  const [timePanel, setTimePanel] = useState<null | "pick" | "drop">(null);
  ...
  const timePickLabel = useMemo(
    () => fmtTimeLabel(timePick),
    [timePick],
  );
  const timeDropLabel = useMemo(
    () => fmtTimeLabel(timeDrop),
    [timeDrop],
  );

  // danh sách mốc thời gian 30 phút
  ...
  const shortGapWarning = useMemo(() => {
    ...
    const diffMin = Math.floor((+dtEnd - +dtStart) / 60000); // tổng phút
    if (diffMin <= 0) return null;

    const DAY_MIN = 24 * 60;
    const fullDays = Math.floor(diffMin / DAY_MIN);
    const remainder = diffMin - fullDays * DAY_MIN; // phút vượt quá mốc tròn ngày

    // Chỉ cảnh báo khi đã qua ít nhất 1 ngày và vượt mốc tròn ngày ≤ 4h
  ...
  return (
    ...
    >
      ...
      <div className="d-flex flex-wrap gap-2 align-items-stretch rent-main-row">
        ...
        {/* Drop-off khi tick “địa điểm khác” */}
        {differentLocation && (
          <div className="flex-grow-1 rent-main-item" style={{ minWidth: 260 }}>
            <div className="h-100 bg-white rounded-3 border d-flex align-items-center px-3 py-2">
              <i className="bi bi-geo-alt fs-5 me-2" />
        ...
        {/* Ngày nhận + giờ nhận */}
        <div className="d-flex align-items-stretch rent-main-item" style={{ minWidth: 220 }}>
          <div
            ref={datePickRef}
          ...
          >
            ...
            <div className="text-truncate">
              ...
              <div className={datePick ? "" : "text-muted"}>{datePickLabel}</div>
            </div>
          </div>
          <div
            ref={timePickRef}
        ...
        <div className="d-flex align-items-stretch rent-main-item" style={{ minWidth: 220 }}>
          ...
          >
            ...
            <div className="text-truncate">
              ...
              <div className={dateDrop ? "" : "text-muted"}>{dateDropLabel}</div>
            </div>
          </div>
          <div
            ref={timeDropRef}
        ...
        {/* Nút tìm */}
        <div className="flex-grow-0 rent-main-item rent-main-search" style={{ minWidth: 140 }}>
          <Button
            variant="primary"
      ...
      {shortGapWarning && (
        ...
        >
          ...
          <div className="mt-1">
            ...
            {shortGapWarning.h > 0 && (
              <strong>{shortGapWarning.h} giờ</strong>
            )}
            {shortGapWarning.m > 0 && (
              <>
                {" "}
