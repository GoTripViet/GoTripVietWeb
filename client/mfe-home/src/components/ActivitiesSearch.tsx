// src/components/ActivitiesSearch.tsx
...
export type ActivitiesSearchResult = {
  destination: string;
  date?: Date;
};

export type ActivitiesSearchProps = {
  onSearch?: (r: ActivitiesSearchResult) => void;
  popular?: { name: string; subtitle?: string }[];
  className?: string;
};

const DEFAULT_POPULAR: NonNullable<ActivitiesSearchProps["popular"]> = [
...
function CalendarSingle({
  ...
  const [base, setBase] = useState(new Date(today.getFullYear(), today.getMonth(), 1)); // tháng đang hiển thị
  const weekdays = ["T2", "T3", "T4", "T5", "T6", "T7", "CN"];
  const cells = buildMonth(base.getFullYear(), base.getMonth());
  const monthLabel = new Date(base.getFullYear(), base.getMonth(), 1).toLocaleDateString("vi-VN", {
    month: "long",
    year: "numeric",
  });
  ...
  return (
    <div style={{ width: "min(360px, 95vw)" }}>
      ...
      <div className="d-grid px-2" style={{ gridTemplateColumns: "repeat(7,1fr)", gap: 6 }}>
        {weekdays.map((w) => (
...
export default function ActivitiesSearch({
  ...
  function submit() {
    ...
    if (destination) setRecent([{ name: destination }, ...recent.slice(0, 4)]);
  }
        ...
        <div className="col-12 col-lg">
          ...
          >
            ...
            <div className="text-truncate">
              ...
              <div className={destination ? "" : "text-muted"}>{destination || "Bạn muốn đi đâu?"}</div>
            </div>
          </div>
        </div>
      ...
      >
        {(props) => (
          <Popover
            ...
            style={{ ...(props as any).style, maxWidth: "none" }}
          >
            <Popover.Header className="d-flex align-items-center justify-content-between">
